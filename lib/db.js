...
export async function listStudents() {
  try {
    ...
    let q = supabase.from("students").select("*").order("display_name");
    if (semId) q = q.eq("semester_id", semId);
    const { data, error } = await q;
    if (error) {
      const msg = String(error.message || "").toLowerCase();
      if (msg.includes("semester_id") || msg.includes("schema cache")) {
        const { data: data2, error: err2 } = await supabase.from("students").select("*").order("display_name");
        if (err2) {
          console.error("[db] listStudents Supabase error:", err2);
          return [];
        }
        return data2 || [];
      }
      console.error("[db] listStudents Supabase error:", error);
      return [];
    }
    return data || [];
  } catch (e) {
    console.error("[db] listStudents failed:", e);
    return [];
  }
}
...
export async function createStudentIfNotExists(name) {
  ...
  let ins = await supabase.from("students").insert({ ...base, name: trimmed }).select("*").single();
  if (ins.error) {
    const msg = String(ins.error.message || "").toLowerCase();
    if (msg.includes("semester_id") || msg.includes("schema cache")) {
      const baseNoSem = { display_name: trimmed, email: fakeMail, auth_user_id: authUserId };
      let ins2 = await supabase.from("students").insert({ ...baseNoSem, name: trimmed }).select("*").single();
      if (ins2.error) ins2 = await supabase.from("students").insert(baseNoSem).select("*").single();
      ins = ins2;
    } else {
